...
def Mean_iou(y_true, y_pred):
   score, up_opt = tf.metrics.mean_iou(y_true, y_pred, 5)
...
def tf_mean_iou(y_true, y_pred, num_classes=5):
...
filelist_trainx = sorted(glob.glob('Inter-IIT-CSRE/The-Eye-in-the-Sky-dataset/sat/*.tif'))
# List of file names of classified images for traininig 
filelist_trainy = sorted(glob.glob('Inter-IIT-CSRE/The-Eye-in-the-Sky-dataset/gt/*.tif'))

# List of file names of actual Satellite images for testing 
filelist_testx = sorted(glob.glob('Inter-IIT-CSRE/The-Eye-in-the-Sky-test-data/sat_test/*.tif'))

filelist_testy = sorted(glob.glob('Inter-IIT-CSRE/The-Eye-in-the-Sky-test-data/gt_test/*.tif'))
                                      

print(len(filelist_trainx))
print(len(filelist_testx))
...
# Padding at the bottem and at the left of images to be able to crop them into 256*256 images for training
...
def new_crops(img, crop_size = 128):
    stride = crop_size 
...
for fname in filelist_trainx[:225]:
...
for fname in filelist_trainy[:225]:
...
#for fname in filelist_trainx[13]:
    ...
    # Reading the image
for fname in filelist_testx[:78]:
    tif = TIFF.open(fname)
    image = tif.read_image()
...
# Reading the image
for fname in filelist_testy[:78]:
    tif = TIFF.open(fname)
    image = tif.read_image()
    image = np.expand_dims(image,3)
    # Padding as required and cropping
...
# tif = TIFF.open('Inter-IIT-CSRE/The-Eye-in-the-Sky-dataset/sat/JAX_004_006_RGB.tif')
# image = tif.read_image()
# crop_size = 128
...
tif = TIFF.open('Inter-IIT-CSRE/The-Eye-in-the-Sky-dataset/gt/JAX_004_006_CLS.tif')
image = tif.read_image()
image = np.expand_dims(image,3)
crop_size = 128
...
#    print(in_shape)
    ...
    model.compile(optimizer = Adam(lr = 0.001), loss = 'categorical_crossentropy', metrics = ['accuracy',Mean_iou])
...
def rgb_to_onehot(rgb_arr, color_dict):
    ...
    for i, cls in enumerate(color_dict):
        n_arr = np.all(np.resize(rgb_arr,(128,128,3)) == color_dict[i], axis=1)
        arr[:,:,i] = np.resize(n_arr,shape[:2])
    return arr
...
history = model.fit(trainx, trainy_hot, epochs=10, validation_data = (testx, testy_hot),batch_size=32, verbose=1)
